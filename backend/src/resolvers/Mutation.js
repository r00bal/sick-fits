...
const Mutations = {
  ...
  async createOrder(parent, args, ctx, info) {
    ...
    const { userId } = ctx.request;
    if (!userId)
      throw new Error('You must be signed in to complete this error');
    ...
    //recalculate the total for the price - super important
    const amount = user.cart.reduce(
      (tally, cartItem) => tally + cartItem.item.price * cartItem.quantity,
      0
    );
    console.log(`Going to charge for a total of ${amount}`);
    //create the strip charge (turn token into money $$$)
    const charge = await stripe.charges.create({
      amount,
      currency: 'USD',
      source: args.token
    });
    //convert the CreateItems to OrderItems
